...
The software-only streamlining is complete. The five tasks below require an
authorized operator on the lab system and retained, reviewable evidence. They
are not performed by repository tests or installation commands.

Never treat a successful status request as permission to move the robot. Use
the fixed lab profile, inspect the generated plan, and require both camera and
robot acknowledgements for capture. The IIWA Stop control only exits an idle
waiting application; it cannot interrupt motion and is not an emergency stop.

## 1. IIWA deployment and pose cadence

- Compile the three current Sunrise applications against the installed
  Sunrise.OS API and deploy the shared structured-command/pose-stream code.
- Verify `/PoseTestBot/TemplateBase`, `/PoseTestBot/PoseTemplateBase`, the nine
  calibration frames, and the ordinary-capture start/end frames in Workbench.
- Simulate every path, then commission it in T1 at reduced override under the
  lab safety process. Do not send the UDP Stop command during calibration.
...
- With explicit authorization and both execution acknowledgements, perform a
  short supervised trial before the full capture.
