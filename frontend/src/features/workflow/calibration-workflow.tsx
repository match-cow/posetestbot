...
type Setup = {
  ...
  solver: {
    ...
    thresholds: {
      ...
      max_nearest_pose_delta_ms: number
      warning_nearest_pose_delta_ms?: number
      warning_pairwise_companion_translation_mm?: number
      warning_pairwise_companion_rotation_deg?: number
      max_pairwise_companion_translation_mm?: number
      max_pairwise_companion_rotation_deg?: number
    }
  }
...
type PromotionBundle = {
  ...
  quality_state: "ok" | "warning"
  quality_warning_count: number
  quality_warnings: PromotionQualityWarning[]
  max_pairwise_companion_translation_mm: number | null
  max_pairwise_companion_rotation_deg: number | null
}
type PromotionReview = {
  policy_revision: string
  status: "promotable" | "promotable_with_warnings" | "blocked"
  selections: Record<string, string>
  selected_camera_count: number
  camera_count: number
  joint_bundle_id: string | null
  selected_bundle: PromotionBundle | null
  eligible_bundles: PromotionBundle[]
  quality_warnings: PromotionQualityWarning[]
  alternative_failure_count: number
...
type Attempt = {
  ...
  promotion_review?: PromotionReview | null
...
export function CalibrationWorkflow({
  ...
  const recommendedOverrides = useMemo(() => {
    const reviewSelections = attempt.data?.promotion_review?.selections ?? {}
    if (Object.keys(reviewSelections).length > 0) return reviewSelections
  ...
  }, [attempt.data?.promotion_review?.selections, attempt.data?.results?.results])
  ...
  return <div className="space-y-5" data-testid="calibration-workflow">
    ...
    {attempt.data?.results && <div className="space-y-4" data-testid="calibration-results">
      <div>
        <h2 className="text-xl font-semibold">Review calibration results</h2>
        <p className="mt-1 text-sm text-muted-foreground">{passingSelections} of {attempt.data.results.results.length} selected cameras have a passing selected solution. Failed alternative solvers remain diagnostic evidence and do not block saving.</p>
      </div>
      {attempt.data.promotion_review && <PromotionReviewCard review={attempt.data.promotion_review} />}
      {attempt.data.results.results.map((result) => <CameraResultCard
        key={result.sensor_key}
        mode={attempt.data.request.mode}
        result={result}
        intrinsicComparison={attempt.data?.intrinsic_comparison?.sensors.find((item) => item.sensor_key === result.sensor_key) ?? null}
        selectedCandidateId={overrides[result.sensor_key] ?? ""}
        onSelect={(candidateId) => setOverrideSelection({ runRoot: selectedRun, attemptId: activeAttemptId ?? "", values: { ...overrides, [result.sensor_key]: candidateId } })}
      />)}
      <Card className="border-primary/30"><CardContent className="flex flex-col gap-4 py-5 sm:flex-row sm:items-center sm:justify-between"><div><div className="font-semibold">Save selected camera calibrations</div><div className="mt-1 text-xs text-muted-foreground">{passingSelections} of {attempt.data.results.results.length} camera profiles selected. Saving is an atomic, auditable promotion; calculation results remain immutable. The saved profiles retain the time-alignment offsets and quality evidence shown above.</div>{attempt.data.promotion_review?.status === "blocked" && <div className="mt-2 text-xs text-destructive">{attempt.data.promotion_review.blocking_reason}</div>}{attempt.data.promotion && ["queued", "running"].includes(attempt.data.promotion.status) && <div className="mt-3 flex flex-col gap-3 rounded-lg border border-primary/30 bg-primary/5 p-3 text-xs sm:flex-row sm:items-center sm:justify-between" data-testid="calibration-promotion-job-status"><p><span className="font-semibold">Calibration saving is {attempt.data.promotion.status}.</span> This background work continues after navigation{attempt.data.promotion.job_id ? <> in job <span className="font-mono">{attempt.data.promotion.job_id}</span></> : null}; Jobs shows its live status and output.</p><Button asChild size="sm" variant="outline" className="shrink-0"><Link to="/jobs">Open Jobs</Link></Button></div>}{attempt.data.promotion?.status === "failed" && <div className="mt-2 text-xs text-destructive">{attempt.data.promotion.error}</div>}{attempt.data.promotion?.status === "promoted" && <div className="mt-2 flex items-center gap-1 text-xs text-success"><CheckCircle2 className="size-4" />Saved {attempt.data.promotion.promoted_profile_ids?.length ?? 0} camera profile(s).</div>}</div><Button size="lg" disabled={passingSelections !== attempt.data.results.results.length || attempt.data.promotion_review?.status === "blocked" || promote.isPending || ["queued", "running", "promoted"].includes(attempt.data.promotion?.status ?? "")} onClick={() => promote.mutate()}>{promote.isPending || ["queued", "running"].includes(attempt.data.promotion?.status ?? "") ? <LoaderCircle className="animate-spin" /> : <Save />}{attempt.data.promotion?.status === "promoted" ? "Calibrations saved" : "Save selected calibrations"}</Button></CardContent></Card>
    </div>}
  </div>
}

function PromotionReviewCard({ review }: { review: PromotionReview }) {
  if (!review.selected_bundle && review.status !== "blocked") return null
  const bundle = review.selected_bundle
  if (review.status === "blocked") return <Card className="border-destructive/40" data-testid="calibration-multi-camera-review"><CardContent className="flex items-start gap-3 py-4 text-xs"><TriangleAlert className="mt-0.5 size-4 shrink-0 text-destructive" /><div><div className="font-semibold text-destructive">Multi-camera promotion is blocked</div><p className="mt-1 text-muted-foreground">{review.blocking_reason}</p><p className="mt-2 text-muted-foreground">Failures from unselected alternative solvers are not blockers; this state means no complete selected bundle satisfies the hard evidence gates.</p></div></CardContent></Card>
  if (!bundle) return null
  const warning = review.status === "promotable_with_warnings"
  return <Card className={warning ? "border-warning/45" : "border-success/35"} data-testid="calibration-multi-camera-review"><CardContent className="flex items-start gap-3 py-4 text-xs">{warning ? <TriangleAlert className="mt-0.5 size-4 shrink-0 text-warning" /> : <CheckCircle2 className="mt-0.5 size-4 shrink-0 text-success" />}<div className="min-w-0"><div className="font-semibold">{warning ? "Promotable with a multi-camera quality warning" : "Multi-camera bundle is promotable"}</div><p className="mt-1 text-muted-foreground">Selected shared bundle <span className="font-mono">{bundle.pnp_method} + {bundle.extrinsic_method}</span>. Fixed-grid estimates differ by {format(bundle.max_pairwise_companion_translation_mm, " mm")} / {format(bundle.max_pairwise_companion_rotation_deg, "°")} across cameras.</p>{warning && <p className="mt-2 text-warning-foreground">The preferred cross-camera limits are 10 mm / 5°. This retained result remains below the 20 mm / 10° hard contradiction limits, so saving is allowed and the warning is preserved with every promoted profile.</p>}{review.alternative_failure_count > 0 && <p className="mt-2 text-muted-foreground">{review.alternative_failure_count} failed alternative solution{review.alternative_failure_count === 1 ? " is" : "s are"} retained for diagnosis; they do not invalidate this selected bundle.</p>}</div></CardContent></Card>
}
...
function CameraResultCard({ mode, result, intrinsicComparison, selectedCandidateId, onSelect }: { mode: Attempt["request"]["mode"]; result: CameraResult; intrinsicComparison: IntrinsicComparison | null; selectedCandidateId: string; onSelect: (value: string) => void }) {
  ...
  const selectedPasses = selected?.status === "passing"
